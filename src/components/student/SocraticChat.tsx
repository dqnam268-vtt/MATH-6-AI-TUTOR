  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-5 items-start">
      ...
      <div className="xl:col-span-8 bg-white rounded-2xl border border-slate-200 shadow-xs flex flex-col h-[760px] overflow-hidden">
        ...
        {/* Quick Chips Prompts & Math Symbols */}
        <div className="bg-slate-50 border-t border-slate-200">
          <div className="px-4 py-2 flex items-center gap-2 overflow-x-auto scrollbar-none border-b border-slate-200/60">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tight flex items-center gap-1 flex-shrink-0">
            ...
            {quickPrompts.map((qp, idx) => (
              <button
                ...
                className="px-3 py-1 bg-white text-slate-700 rounded-full text-xs font-semibold hover:bg-blue-50 hover:text-blue-700 border border-slate-200 transition-all whitespace-nowrap shadow-2xs"
              >
                {qp}
              </button>
            ))}
          ...
          {/* Quick Math 6 Symbol Toolbar */}
          <div className="px-4 py-1.5 flex items-center gap-1.5 overflow-x-auto scrollbar-none bg-slate-100/70">
            <span className="text-[10px] font-bold text-blue-700 uppercase tracking-tight flex-shrink-0 mr-1">
              Ký hiệu:
            </span>
            {[
              { label: '· (nhân)', sym: ' \\cdot ' },
              ...
              { label: '∈ ℕ', sym: ' \\in \\mathbb{N}' },
              { label: '≤', sym: ' \\le ' },
              { label: '≥', sym: ' \\ge ' },
              { label: '≠', sym: ' \\ne ' },
              { label: '° (độ)', sym: '^\\circ' },
              { label: '∠ (góc)', sym: ' \\widehat{AOB} ' },
              ...
              <button
                key={idx}
                type="button"
                onClick={() => setInput((prev) => prev + s.sym)}
                className="px-2 py-0.5 bg-white text-slate-700 hover:bg-blue-600 hover:text-white rounded text-[11px] font-mono font-bold border border-slate-200 transition-colors flex-shrink-0 shadow-2xs"
              >
                {s.label}
              </button>
            ))}
